...
describe('Pathogen - Follow Up Integration Tests', () => {
  [NotificationType.FollowUpNotification7_1, NotificationType.FollowUpNotification7_3].forEach(notificationType => {
    describe(`${notificationType}`, () => {
      ...
      const expectedHeaderText =
        notificationType === NotificationType.FollowUpNotification7_1
          ? 'Meldung eines Nachweises von Krankheitserregern gemäß § 7 Abs. 1 IfSG ohne Angaben von Personendaten'
          : 'Meldung eines Nachweises von Krankheitserregern gemäß § 7 Abs. 3 IfSG ohne Angaben von Personendaten';
      const is7_1 = notificationType === NotificationType.FollowUpNotification7_1;

      beforeEach(() => buildMock(true, notificationType));

      beforeEach(() => {
        const result = setupIntegrationTests({
          ...mainConfig,
          featureFlags: { ...mainConfig.featureFlags },
        });
      ...
      it('should show follow up header', async () => {
        await waitForStability(fixture);
        const textContent = fixture.nativeElement.textContent;
        expect(textContent.includes(expectedHeaderText)).toBeTrue();
      });

      it('should show follow up input dialog', async () => {
      ...
      describe('Tests after positive Pop-Up Validation', () => {
        ...
        it('should show the follow-up disclaimer text on notified person page', async () => {
          ...
          if (is7_1) {
            ...
            expect(fixture.nativeElement.textContent).toContain(NotifiedPersonDisclaimer.FOLLOW_UP_NONNOMINAL_DISCLAIMER);
          }
        });
      ...
      describe('Mixed Follow-Up (FEATURE_FLAG_MIXED_FOLLOW_UP enabled)', () => {
        ...
        it('should call fetchFollowUpCode when ID is validated and hasValidNotificationId$ emits true', async () => {
          ...
          expect(document.querySelector('.mat-mdc-dialog-container')).toBeNull();
          expect(fhirService.fetchFollowUpCode).toHaveBeenCalledWith('invp', notificationType);
        });
